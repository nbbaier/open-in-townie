async function openTownie() {
	try {
		...
		// Button text and state should not be changed here as per requirements.
		...
		if (!response.ok) {
			...
			// Button text and state should not be changed here.
			return;
		}
...
function addTownieButton() {
	...
	if (townieButton) { // Button already exists
		...
		townieButton.style.visibility = 'visible'; // Ensure existing button is visible
	...
	townieButton.style.visibility = 'hidden'; // Initially hide the button
	...
	if (remixButton.parentNode === parentDiv) { // Ensure parentDiv is indeed the direct parent
		...
		townieButton.style.visibility = 'visible'; // Make button visible after placement
		console.log("Open in Townie button added next to Remix button and made visible.");
	} else {
		console.error("Remix button's parent is not the identified target div. Button not added.");
		// If not added, no need to make it visible.
	}
}
